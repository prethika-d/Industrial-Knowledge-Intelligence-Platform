...
export default function UploadPage() {
  ...
  const addFiles = useCallback((fileList) => {
    const incoming = Array.from(fileList).map((file) => ({
      ...
      name: file.name,
      size: file.size,
      status: 'queued', // queued | uploading | done
    }))
      ...
      >
        {files.length === 0 ? (
          <p className="text-sm text-paper-500 py-6 text-center">
            No files yet. Uploaded documents will appear here before processing.
          </p>
        ) : (
          <div className="space-y-2">
            {files.map((f) => (
              ...
              >
                <FiFile size={16} className="text-paper-500 shrink-0" />
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-paper-100 truncate">{f.name}</p>
                  <p className="readout text-[11px] text-paper-500">{formatSize(f.size)}</p>
                </div>
